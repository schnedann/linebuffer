    ...
    uint64 *hash2)
{
    ...
    u.p8 = static_cast<uint8 const *>(message);
    ...
    uint64 *hash2)
{
    ...
    remainder = static_cast<size_t>(length - static_cast<size_t>(reinterpret_cast<uint8 const *>(end)-static_cast<uint8 const *>(message)));
...
void SpookyHash::Update(const void *message, size_t length)
{
    ...
    if (m_remainder)
    {
        ...
        u.p8 = (reinterpret_cast<uint8 const *>(message)) + prefix;
        length -= prefix;
    }
    else
    {
        u.p8 = reinterpret_cast<uint8 const *>(message);
    }

    // handle all whole blocks of sc_blockSize bytes
    end = u.p64 + (length/sc_blockSize)*sc_numVars;
    remainder = static_cast<uint8>(length-(reinterpret_cast<uint8 const *>(end)-u.p8));
    if (ALLOW_UNALIGNED_READS || (u.i & 0x7) == 0)
    {
        while (u.p64 < end)
        {
...
void SpookyHash::Final(uint64 *hash1, uint64 *hash2)
{
    ...
    const uint64 *data = reinterpret_cast<uint64 const *>(m_data);
