...
TEST_CASE("Math::Discrete","[math discrete]"){
  ...
  SECTION("Math::nextpow2"){
    ...
    for(u32 ii=0; ii<0x8001U; ++ii){
      ...
      if(is_nextpower){
        nextpower = Math::Boolean::Logic_Shift_L<u32>(nextpower,1);
      }
    }
  }

  SECTION("Math::Discrete::odd"){
    auto const _x = u16(GENERATE(take(150, random(0, 65535))));

    auto dut = Math::Discrete::is_odd<u16>(_x);
    auto ref = (0 != (_x%2));
  ...
  SECTION("Math::Discrete::even"){
    ...
    auto dut = Math::Discrete::is_even<u16>(_x);
    auto ref = (0 == (_x%2));
  ...
  SECTION("Math::Discrete::leadingzeros"){
    u8 border = 0;
    for(u64 ii=0; ii<Math::Boolean::MASK_MSB<u64>(); ii=1+(ii*2)){
      CAPTURE(ii);
      REQUIRE( u16(64-border) == u16(Math::Discrete::leadingzeros<u64>(ii)) );
      ++border;
    }
  }
  ...
  SECTION("Math::Discrete::udivceil<u16>"){
    ...
    auto ref2 = u16(std::ceil(double(a)/double(b)));
  ...
  SECTION("Math::Discrete::divide_by_shift<u16> with rounding"){
    ...
    auto ref2 = std::round(double(a)/double(b));
